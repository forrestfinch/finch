...
$(window).on('resize', function() {
  ...
  mainSectionHeight = $('#finch-os').outerHeight() - headerHeight;
  //remove the height in the container of the main section 'home'
  $('#finch-os').css('height', '')
  containerHeight = $('#finch-os').outerHeight() + 30
})
...
// tumbleweed finch more information
...
function osMoreInformation(os) {
  ...
  containerHeight = $('#finch-os').outerHeight();
  ...
  function hideLandingPageInfo() {
    ...
    $('#finch-os .container-fluid').hide()
  }

  function createMoreInformationArea() {
    information = '<div class="text-center" id="more-information-os">' + '<div class="os-icon"></div>' + '<h1 class="wow fadeInUp">' + osSelectedTitle + '</h1>' + '<div class="wow fadeInUp">' + osSelectedText + '</div>' + '<br/>' + '<div class="btn btn-link back-to-main-page">' + '<i class="fa fa-long-arrow-left"></i>' + '<span lang="en">Back to main page</span>' + '</div>' + '</div>'
    $('#finch-os').append(information)
    ...
    $('#home, #finchLogo').on('click', function() {
      history.pushState('', document.title, window.location.pathname);
      scrolled = $(window).scrollTop();
      if (scrolled > mainSectionHeight) {
...
function backToMainPageOs(os) {
  ...
  function showMainInformation() {
    $('#finch-os .container-fluid').removeClass('animated bounceOut').show().addClass('animated bounceIn')
  }
  ...
  function bounceBall(element, times, speed) {
    ...
    var openContributionDetails = function() {
      ...
      $("#contribute-details").css({
          background: 'rgba(109, 167, 65, 0.9)'
        })
...
$(document).on("ready", function() {
  if (!Modernizr.svg) {
    ...
    $("#contribute-to-finch").addClass('no-svg')
  }
})
